...
function Login({ onLogin }) {
  ...
  const [resetToken, setResetToken] = useState(''); // Only for demonstration/hackathon purposes
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      ...
      setResetToken(data.resetToken); // Displaying it for easier testing
    } catch (error) {
      const { toast } = await import('react-toastify');
      toast.error(error.message || 'Error generating reset link');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className={`relative w-full max-w-6xl transition-all duration-1000 transform ${
        ...
        <div className="grid lg:grid-cols-2 gap-0 bg-white/5 backdrop-blur-xl rounded-3xl shadow-2xl overflow-hidden border border-white/10">
          ...
          <div className="bg-white backdrop-blur-sm p-8 lg:p-12">
            ...
            {isForgotPassword ? (
              <form onSubmit={resetToken ? handleResetPassword : handleForgotPassword} className="space-y-5">
                {!resetToken ? (
                  <div className="space-y-1.5">
                    ...
                    <div className="relative group">
                      ...
                      <input
                        ...
                        className={`w-full pl-10 pr-4 py-3 border rounded-xl focus:outline-none focus:ring-2 ${emailError ? 'border-red-300' : 'border-gray-200'}`}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative group">
                      ...
                      <input
                        ...
                        className={`w-full pl-10 pr-4 py-3 border rounded-xl focus:outline-none focus:ring-2 ${passwordError ? 'border-red-300' : 'border-gray-200'}`}
                ...
                <button
                  type="submit"
                  disabled={isLoading}
                  className="w-full py-3.5 px-4 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-all shadow-lg"
                >
                  {isLoading ? 'Processing...' : (resetToken ? 'Reset Password' : 'Send Reset Link')}
                </button>
